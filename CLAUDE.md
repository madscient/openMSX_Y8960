...
**出典どうしの食い違いを主張するときは、名前ではなく挙動を決めている行まで辿る。**
2026-09-09 に「OPLL の番号が RTL と逆」と誤って断定した。localparam の名前だけを
見て、回路を選んでいるのが `bus_address[1]` だという 2 行を見ていなかった。
行番号を引用していても、それが名前を定義する行なら証拠にならない。
