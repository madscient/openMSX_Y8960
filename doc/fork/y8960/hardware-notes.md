...
一次情報は3系統あり、**互いに食い違っている**。優先順位は
`fpga/.../src/*.v` (RTL) > `doc/spec/*.xlsx` > `doc/manual/*.docx`
とする。RTL が唯一「動く形」で書かれているため。食い違いは §7 に列挙する。
...
分解能は 0-7 の 8 段（71.5us 〜 1.17s、`src/timer/doc/msx_timer.xlsx`）。
