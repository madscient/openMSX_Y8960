...
2026-09-09 時点で 20 ファイル（**確認済み**: `git diff --stat upstream/master`）。
...
| **使われていない** | `src/sound/MSXPSG.{cc,hh}` |

**`MSXPSG` の `<chip_select>` はどの構成からも使われていない**（**確認済み**:
`share/` を grep して 0 件）。buppu3 実装が Y8960 の SSG のために足したものだが、
こちらは `Y8960SSGSDevice` を別に作ったので出番が無い。
消せば upstream ファイルが 2 つ減る。**保留**（判断を仰いでいない）。

**`openmsx.vcxproj` の差分 3452 行は、実質 22 行しかない。** 残りは改行コードの
差である（**確認済み**: `--ignore-all-space` を付けると 22 行になる）。
このままだと上流が触った瞬間に全面衝突する。直すなら改行を上流に揃える。
やり直しの値段はこの 1 ファイルで、コードには波及しない。**保留**。
