...
### 3.6.2 SSGS の GPIO【未実装】

本体内蔵版の SSGS は **GPIO を持つ**。カートリッジ版には無い。

YMZ 系そのものに I/O ポートは無く、`Y8960SsgCore` は `AY8910` から
periphery とレジスタ `$0E`/`$0F` を落としてある（§3.6）。
本体内蔵版はそれを戻すことになる。

**確認事項（hra1129 さんへ）**。

- **2 系統のうちどちらが GPIO を持つのか。** 本体 PSG を置き換える側だけか、
  両方か。レジスタ番号は各系統の `$0E`/`$0F` でよいか
- **GPIO の先に何が繋がるのか。** 本体 PSG を置き換えるなら、
  ジョイスティック 2 ポート・かなランプ・カセットのモータ制御という
  MSX の割り当てをそのまま引き継ぐと考えてよいか

**やり直しの値段**。上の 2 点が決まらないと実装できない。決まった後は、
`Y8960SsgCore` にレジスタ `$0E`/`$0F` と方向ビット（`$07` の b6/b7）を戻し、
openMSX 側の `AY8910Periphery` に相当する結線を `Y8960SSGSDevice` に足す。
既存の `MSXPSG` がその結線を持っているので、参照先はある。
**カートリッジ版には波及しない**（GPIO 無しが既定）。
